...
export function TestMode({ deck, initialFilter = "all" }: { deck: Deck; initialFilter?: ContentFilter }) {
  ...
  function automatic(question: Question, answers: Record<string, string>) { const answer = answers[question.id] ?? ""; return question.type === "written" ? gradeWrittenAnswer(answer, question.expected, !strict) : answer === question.expected; }
  ...
  if (!exam) return <div className="mx-auto max-w-2xl space-y-6"><header><p className="text-sm text-accent">{deck.title}</p><h1 className="font-display text-3xl">Set up Test</h1><p className="mt-2 text-muted">Check what you know. Results stay hidden until submission.</p></header>
    ...
    <Section title="Answer direction"><DirectionSelect value={direction} onChange={setDirection} /></Section><Section title="Grading and order"><Toggle label="Spelling must be correct" value={strict} onChange={setStrict} /><p className="text-sm text-muted">When off, conservative local grading allows minor typos.</p><Toggle label="Automatically grade test" value={auto} onChange={setAuto} /><Toggle label="Shuffle questions" value={shuffle} onChange={setShuffle} /></Section>
