...
import { filterCards, emptyContent, studyUrl, type ContentFilter, type Direction } from "@/lib/learning/content";
...
export function TestMode({ deck, initialFilter = "all" }: { deck: Deck; initialFilter?: ContentFilter }) {
  const { prefs, setStudyPreferences } = useStore();
  const saved = prefs.study?.test;
  const [filter, setFilter] = useState(initialFilter), [count, setCount] = useState(saved?.count ?? "all"), [custom, setCustom] = useState(saved?.custom ?? 5);
  const [types, setTypes] = useState<QuestionType[]>(saved?.types ?? ["choice", "written"]), [direction, setDirection] = useState<Direction>(saved?.direction ?? "term");
  const [strict, setStrict] = useState(saved?.strict ?? true), [auto, setAuto] = useState(saved?.auto ?? true), [shuffle, setShuffle] = useState(saved?.shuffle ?? true);
  const [smart, setSmart] = useState(saved?.smart ?? false);
  ...
  }).catch(() => setError("The saved Test session could not be opened. You can start a new test safely.")); }, [deck.id, deck.cards]);
  useEffect(() => {
  ...
  function changeFilter(next: ContentFilter) {
    const available = filterCards(deck.cards, next).length;
    setFilter(next);
    if (count === "custom" && custom > available) setCustom(Math.max(1, available));
    else if (count !== "all" && count !== "custom" && Number(count) > available) setCount("all");
  }
  function start(ids?: string[]) {
    ...
    void setStudyPreferences("test", { count, custom, types, direction, strict, auto, shuffle, smart });
  ...
  if (!exam) return <div className="mx-auto max-w-2xl space-y-6"><header><p className="text-sm text-accent">{deck.title}</p><h1 className="font-display text-3xl">Set up Test</h1><p className="mt-2 text-muted">Check what you know. Results stay hidden until submission.</p></header>
    ...
    <Section title="Content"><ContentSelect value={filter} onChange={changeFilter} /><label className="block text-sm">Number of questions<select className={fieldClass} value={count} onChange={e => setCount(e.target.value)}>{[5, 10, 20].map(n => <option key={n} value={n} disabled={n > eligible.length}>{n}</option>)}<option value="custom">Custom</option><option value="all">All ({eligible.length})</option></select></label>{count === "custom" && <label className="block text-sm">Custom question count<input className={fieldClass} type="number" min={1} max={eligible.length} value={custom} onChange={e => setCustom(Math.max(1, Math.min(Number(e.target.value), eligible.length)))} /></label>}<p className="text-sm text-muted">{eligible.length} eligible cards. No repeated questions.</p></Section>
    ...
    <Section title="Answer direction"><DirectionSelect value={direction} onChange={setDirection} /></Section><Section title="Grading and order">{types.includes("written") && <details open className="rounded-2xl border border-edge p-4"><summary className="min-h-11 cursor-pointer font-medium">Written answer grading</summary><div className="mt-3 space-y-3"><Toggle label="Spelling matters" value={strict} onChange={setStrict} /><Toggle label="Smart Grading" value={smart} onChange={setSmart} /><p className="text-sm text-muted">Spelling tolerance handles minor typos. Smart Grading accepts author-approved equivalent answers and never guesses meaning.</p></div></details>}<Toggle label="Automatically grade test" value={auto} onChange={setAuto} /><Toggle label="Shuffle questions" value={shuffle} onChange={setShuffle} /></Section>
  ...
  if (exam.phase === "results") { const total = exam.questions.length, skipped = exam.questions.filter(question => !exam.answers[question.id]?.trim()).length, incorrect = Math.max(0, exam.history.incorrect - skipped), incorrectIds = exam.questions.filter(question => !exam.grades[question.id]).map(question => question.cardId); return <div className="mx-auto max-w-3xl space-y-6"><h1 className="font-display text-3xl">Test results</h1><Metrics values={[["Percentage", `${Math.round(exam.history.correct / total * 100)}%`], ["Correct", exam.history.correct], ["Incorrect", incorrect], ["Unanswered", skipped], ["Total questions", total], ["Time", durationLabel(exam.history.durationMs)]]} />
    <div className="flex flex-wrap gap-3"><Button onClick={() => start(exam.questions.map(question => question.cardId))}>Retake Test</Button>{incorrectIds.length > 0 && <><Button variant="secondary" onClick={() => start(incorrectIds)}>Test Incorrect</Button><LocalLink className="study-link" href={studyUrl(deck.id, "learn", incorrectIds)}>Learn Incorrect</LocalLink><LocalLink className="study-link" href={studyUrl(deck.id, "flashcards", incorrectIds)}>Flashcards for Incorrect</LocalLink></>}<LocalLink className="study-link" href={studyUrl(deck.id, "test", undefined, "starred")}>Study Starred</LocalLink><LocalLink className="study-link" href={`/library/${deck.id}`}>Back to Deck</LocalLink></div>
    <Section title="Detailed review"><ol className="divide-y divide-edge">{exam.questions.map((question, i) => { const card = deck.cards.find(card => card.id === question.cardId); return <li key={question.id} className="space-y-2 py-5"><div className="flex justify-between gap-3"><h3 className="whitespace-pre-wrap break-words font-medium">{i + 1}. {question.prompt}</h3>{card && <StarButton card={card} />}</div>{question.pairing && <p className="text-muted">Paired with: {question.pairing}</p>}<p className="text-sm">Your answer: {exam.answers[question.id] || "Unanswered"}</p><p className="text-sm">Correct answer: {question.expected}</p><p className={exam.grades[question.id] ? "text-accent" : "text-danger"}>{exam.grades[question.id] ? "Correct" : "Incorrect"}</p></li>; })}</ol></Section></div>; }
  ...
  return <div className="mx-auto max-w-3xl space-y-6"><header className="flex items-start justify-between gap-4"><div><p className="text-sm text-accent">{deck.title} · Test</p><h1 className="font-display text-2xl">Question {index + 1} of {exam.questions.length}</h1></div><LocalLink className="study-link" href={`/library/${deck.id}`}>Save & Exit</LocalLink></header><ProgressBar label={`${exam.questions.length - unanswered} answered`} value={(exam.questions.length - unanswered) / exam.questions.length * 100} />
