...
export function MatchMode({ deck, initialFilter = "all" }: { deck: Deck; initialFilter?: ContentFilter }) {
  ...
  const playable = eligible.filter(card => eligible.filter(other => other.term.trim().toLocaleLowerCase() === card.term.trim().toLocaleLowerCase()).length === 1 && eligible.filter(other => other.definition.trim().toLocaleLowerCase() === card.definition.trim().toLocaleLowerCase()).length === 1);
  const omitted = eligible.length - playable.length;
  const count = pairs === "all" ? Math.min(12, playable.length) : Math.min(Number(pairs), playable.length);
  function start() {
    if (!eligible.length) return;
    const cards = (shuffle ? shuffled(playable) : playable).slice(0, count);
  ...
  if (!game) return <div className="mx-auto max-w-2xl space-y-6"><header><p className="text-sm text-accent">{deck.title}</p><h1 className="font-display text-3xl">Set up Match</h1><p className="mt-2 text-muted">Tap a term, then its definition. No dragging needed.</p></header><Section title="Content"><ContentSelect value={filter} onChange={setFilter} /></Section><Section title="Number of pairs"><label className="block text-sm">Pairs<select className={fieldClass} value={Number(pairs) > playable.length ? "all" : pairs} onChange={e => setPairs(e.target.value)}>{[6, 8, 10].filter(n => n <= playable.length).map(n => <option key={n} value={n}>{n}</option>)}<option value="all">{playable.length > 12 ? `One board (12 of ${playable.length})` : `All (${playable.length})`}</option></select></label></Section><Toggle label="Shuffle card selection" value={shuffle} onChange={setShuffle} />{omitted > 0 && <p role="status" className="rounded-xl bg-surface-2 p-3 text-sm text-muted">{omitted} ambiguous {omitted === 1 ? "card was" : "cards were"} left out because its term or definition is duplicated.</p>}{!eligible.length && <p role="status">{emptyContent(filter)}</p>}<div className="flex gap-3"><Button size="lg" disabled={!playable.length} onClick={start}>Start Match</Button><LocalLink className="study-link" href={`/library/${deck.id}`}>Back to Deck</LocalLink></div></div>;
  ...
  return <div className="space-y-5"><header className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm text-accent">{deck.title}</p><h1 className="font-display text-3xl">Match</h1></div><LocalLink className="study-link" href={`/library/${deck.id}`}>Back to Deck</LocalLink></header><Metrics values={[["Remaining pairs", game.pairs - game.removed.length / 2], ["Mistakes", game.history.incorrect], ["Time", durationLabel(elapsed)]]} /><p role="status" className="min-h-6 text-sm text-muted">{feedback || "Choose two tiles that belong together."}</p>
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-4">{game.tiles.map(tile => { const card = deck.cards.find(card => card.id === tile.cardId); const removed = game.removed.includes(tile.id); return <article key={tile.id} className={`relative flex min-w-0 flex-col rounded-2xl border p-3 ${removed ? "border-edge bg-surface-2 opacity-50" : game.selected === tile.id ? "border-accent bg-accent/5" : "border-edge bg-surface"}`}><div className="flex items-center justify-between"><span className="text-xs text-muted">{removed ? "Matched" : tile.side === "term" ? "Term" : "Definition"}</span>{card && <StarButton card={card} />}</div><button type="button" disabled={busy || removed} aria-pressed={game.selected === tile.id} aria-label={`${tile.side}: ${tile.text}`} onClick={() => void handleSelect(tile)} className="min-h-24 flex-1 whitespace-pre-wrap break-words rounded-lg p-2 text-left text-base leading-relaxed focus-visible:outline-2 focus-visible:outline-accent">{tile.text}</button></article>; })}</div>{error && <p role="alert" className="text-danger">{error}</p>}</div>;
}
