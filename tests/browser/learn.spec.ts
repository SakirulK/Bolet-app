...
for (const [filter, count, first] of [['all', 6, 'CPU'], ['starred', 3, 'CPU']] as const) {
...
for (const direction of ['term', 'definition', 'mixed']) test(`Learn ${direction} answer direction and card star`, async ({ page }) => {
  await seedStudy(page, 2); await openMode(page, 'learn'); await setupLearn(page); await page.getByLabel('Answer with').selectOption(direction);
  await page.getByRole('button', { name: 'Start Learning' }).click();
  ...
  await page.getByRole('button', { name: 'Unstar card', exact: true }).click();
  await page.getByRole('button', { name: 'Star card', exact: true }).click();
  await expect(page.getByRole('button', { name: 'Unstar card', exact: true })).toHaveAttribute('aria-pressed', 'true');
  const first = (await records<Card>(page, 'cards')).find(card => card.id === 'c0')!; expect(first.starred).toBe(true);
  await continueLearn(page); await expect(page.getByRole('heading', { level: 2 })).toHaveText(direction === 'term' ? 'Organelle' : definitions[1]);
  await page.reload(); const persisted = (await records<Card>(page, 'cards')).find(card => card.id === 'c0')!; expect(persisted.starred).toBe(true);
});
test('Learn Don’t Know keeps card starring available, has no override and returns after other cards', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Unstar card', exact: true }).click(); await continueLearn(page);
...
test('Learn override replaces rejection, persists once, updates accuracy and avoids miss requeue', async ({ page }) => {
  ...
  const [card] = await records<Card>(page, 'cards'); expect(card.incorrectCount).toBe(0); expect(card.correctStreak).toBe(1); expect(card.starred).toBe(false);
});
test('Learn retype reinforces without erasing incorrect result; missed card returns', async ({ page }) => {
  ...
  await learnAnswer(page, 'wrong'); await page.getByRole('button', { name: 'Unstar card', exact: true }).click(); await expect(page.getByRole('button', { name: 'Star card', exact: true })).toHaveAttribute('aria-pressed', 'false'); await page.getByRole('button', { name: 'Retype Answer', exact: true }).click();
...
test('Learn multiple choice, recommendation bounds and empty starred set', async ({ page }) => {
  ...
  await page.goto('/practice?deck=learn-deck&mode=learn&filter=starred&ids=c3');
  await expect(page.getByText('No starred cards yet.', { exact: true })).toBeVisible(); await expect(page.getByRole('button', { name: 'Start Learning' })).toBeDisabled();
});
