...
test('Match pairs, mistakes, card stars, rapid taps, completion and persistence', async ({ page }) => {
  ...
  await termTile.getByRole('button', { name: 'Unstar card' }).click(); await definitionTile.getByRole('button', { name: 'Star card' }).click();
  ...
  await page.reload(); const card = (await records<Card>(page, 'cards')).find(card => card.id === 'c0')!; expect(card.starred).toBe(true);
});
test('Match starred filter retains every selected pair', async ({ page }) => {
  await seedStudy(page, 12); await openMode(page, 'match', '&filter=starred'); await page.getByRole('button', { name: 'Start Match' }).click();
  await metric(page, 'Remaining pairs', '3'); await expect(page.getByRole('button', { name: /^term:/ })).toHaveCount(3); await expect(page.getByRole('button', { name: /^definition:/ })).toHaveCount(3);
  const labels = await page.getByRole('button', { name: /^term:/ }).allTextContents(); expect(new Set(labels).size).toBe(3);
});
