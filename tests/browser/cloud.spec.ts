...
test('local migration, profile, persisted session, sign-out safety and new-device recovery',async({page,context,browser})=>{
  ...
  await page.goto('/profile');await page.getByRole('button',{name:'Sign out'}).click();await expect(page.getByRole('heading',{name:'Study anywhere.'})).toBeVisible();await expect(page.getByText('Study data is still stored on this device.')).toBeVisible();expect((await records(page,'decks')).length).toBe(1);
